...
bool av_load_ffmpeg(void) {
    ...
    for (int i = 0; version_sets[i].avcodec != NULL; i++) {
        ...
        if (libavcodec && libavformat && libavutil) {
            ...
            LOAD_SYM(libavcodec, avcodec_find_encoder);
            LOAD_SYM(libavcodec, avcodec_send_frame);
            LOAD_SYM(libavcodec, avcodec_receive_packet);
            ...
            LOAD_SYM(libavformat, avformat_alloc_output_context2);
            LOAD_SYM(libavformat, avformat_new_stream);
            LOAD_SYM(libavformat, avformat_write_header);
            LOAD_SYM(libavformat, av_interleaved_write_frame);
            LOAD_SYM(libavformat, av_write_trailer);
            LOAD_SYM(libavformat, avformat_free_context);
