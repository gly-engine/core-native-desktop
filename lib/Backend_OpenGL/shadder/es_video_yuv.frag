...
void main() {
    ...
    // Brightness/contrast (identity: brightness=1.0, contrast=1.0)
    color = color * u_contrast + (u_brightness - 0.5 * u_contrast - 0.5);
    color = clamp(color, 0.0, 1.0);

    // Saturation via mix, no branch (identity: saturation=1.0)
    float gray = dot(color, vec3(0.299, 0.587, 0.114));
    color = mix(vec3(gray), color, u_saturation);

    // Film grain (u_film_grain=0 -> adds 0)
    float n = rand(uv + u_time * 0.1) * u_film_grain;
    color += n - u_film_grain * 0.5;

    // Scratch line via step() (branchless, u_scratch=0 -> adds 0)
