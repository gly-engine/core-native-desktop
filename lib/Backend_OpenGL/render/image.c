...
static void gl_upload(int32_t id, void **backend_data,
    ...
    /* Only reachable on a GLES context that reported the ETC1 extension: the
     * etc1 image_backend is registered only when etc1_supported (see
     * gamely_daemon_img_opengl_register), so nothing can decode to this format
     * otherwise. glad/gl.h declares the token and entry point for both APIs. */
